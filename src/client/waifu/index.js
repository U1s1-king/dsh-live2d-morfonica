...
async function loadWidget(hooks) {
    ...
    for (const tool of getConfig().tools) {
        ...
        toolBar.insertAdjacentHTML("beforeend",
            `<span id="waifu-tool-morfonica-${tool}" title="${TOOL_TITLES[tool] || tool}">${decodeURIComponent(icon).replace("data:image/svg+xml,", "")}</span>`);
    ...
    hooks.on(document, "click", event => {
        if (event.target.closest("#model-selection-panel-morfonica") ||
            event.target.closest("#texture-selection-panel-morfonica") ||
            event.target.closest("#waifu-tool-morfonica") ||
            event.target.closest("#waifu-toggle-morfonica")) {
            return;
        }
        closePanels();
    });
...
function enableDrag(widgetEl) {
    ...
    widgetEl.addEventListener("pointerdown", event => {
        if (event.target.closest("#waifu-tool-morfonica") ||
            event.target.closest(".waifu-panel-morfonica") ||
            event.target.closest("#waifu-toggle-morfonica")) {
            return;
        }
        drag.active = true;
    ...
    const endDrag = event => {
        ...
        if (drag.moved) {
            const rect = widgetEl.getBoundingClientRect();
            try {
                localStorage.setItem("morfonica-waifu-pos", JSON.stringify({ left: rect.left, top: rect.top }));
            } catch (error) { /* 忽略 */ }
        }
    };
...
async function initWidget(config) {
    ...
    if (localStorage.getItem("morfonica-waifu-display") && Date.now() - localStorage.getItem("morfonica-waifu-display") <= 86400000) {
